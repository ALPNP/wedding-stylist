...
//            echo $imagesUtf8;
        ...
        {
            $stateAddFalse = 3;
            $publicState = "INSERT INTO `states` ( `id`, `cat`, `podcat`, `header`, `images`, `statetext`, `status`)
                    VALUES ('".$id."', '".$cat."', '".$podcat."', '".$header."', '".$imagesUtf8."', '".$stateText."', '".$stateAddFalse."')";
        }
        else
        {
            $publicState = "INSERT INTO `states` ( `id`, `cat`, `podcat`, `header`, `images`, `statetext`, `status`)
                    VALUES ('".$id."', '".$cat."', '".$podcat."', '".$header."', '".$imagesUtf8."', '".$stateText."', '".$public."')";
        }
    
        $db->query($publicState);
    ...
    {
        echo "<p>Статья была успешно опубликована.</p>";
        echo "<input class='buttonSend' onclick='window.history.back();' type='button' value='Вернуться обратно'/>";
        header('Refresh: 5; URL=http://localhost/php/app/action.php');
    }
    exit();
